...
if (!function_exists('get_products_json_by_url')) {
    function get_products_json_by_url() {
        if (isset($_GET['action']) && $_GET['action'] === 'np_route_products_json') {
            ...
            $categoryLinkFormat = isset($_GET['np_from']) && $_GET['np_from'] == 'theme' ? '?products-list&category_id=' : '?productsList&categoryId=';
            if (isset($data['categories']) && $data['categories']) {
                foreach ($data['categories'] as $i => $category) {
                    if (!isset($data['categories'][$i]['link']) && isset($category['id'])) {
                        $data['categories'][$i]['link'] = home_url($categoryLinkFormat . $category['id']);
                    }
                }
            }
